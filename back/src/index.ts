...
//import { EmailListResolver } from "./resolvers/emailList";
...
let importAgeValue: any = process.env.COOKIE_MAXAGE;
...
const main = async () => {
    // I will not be automating the migrations process as I would rather manually handle migrations.
    if( process.env.IS_LOCAL_HOST === "false" ) { 
        corsOrigin = process.env.CORS_ORIGIN_DOMAIN; 
        runningOn = process.env.GRAPH_QL_RUNNING_ON; 
        } else { 
            corsOrigin = process.env.CORS_ORIGIN_LOCAL; 
            runningOn = process.env.GRAPH_QL_RUNNING_ON_LOCAL_HOST;
        } //used to setup for local vs domain cors   
    try {
        const orm = await MikroORM.init(mikroConfig); 
         ...
         const apolloServer = new ApolloServer({ 
             schema: await buildSchema({resolvers: [PostResolver, UserResolver, ProjectResolver, 
                //EmailListResolver
            ], validate: apolloValid}),
             // @ts-ignore  
...
function loadStatement(){
    ...
    console.log('CORS Origin: ', corsOrigin,', and ',trustProxy);
    ...
    console.log(' ');
    console.log('grpahql running on : ', runningOn + "/graphql using port : " + process.env.EXPRESS_PORT);
    console.log('  ');
}
