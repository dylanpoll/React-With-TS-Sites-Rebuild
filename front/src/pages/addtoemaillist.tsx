...
// eslint-disable-next-line no-empty-pattern
const AddToEmailList: React.FC<registerProps> = ({}) => {
const router = useRouter();
const [, register] = useEmailListQuerryMutation();
    return (
      <>    
        <Navigation/> 
        <Wrapper>  
            <Formik 
                initialValues={{ name: "", class: "", email:"" }} 
                    onSubmit={async ( values, {setErrors} ) => {
                        //console.log(values);
                        const response = await register({options: values});
                        if (response.data?.register.errors){ //if there are errors, no argument present makes this a true false for if the data is present or null
            ...
            >
              ...
              <Box mt={2}>
                ...
                <Button mt={2} type="submit" isLoading={isSubmitting} > Add To Email List </Button>
              </Form>
            )}
          </Formik>
        </Wrapper>
...
export default withUrqlClient(createUrqlClient)(AddToEmailList);
